...
edu_datasets <- cbs_search("opleidingsniveau")
View(edu_datasets)

# Search for income datasets
inc_datasets <- cbs_search("inkomen provincie")
View(inc_datasets)

# from the package
dataset <- cbs_get_data("84364NED")

saveRDS(dataset, "data/data_8436NED.rds")

# Eurostat
dataset_eurostat <- get_eurostat("tesem180")
...
### select colomns ####
dataset_adjusted <- dataset %>% 
  select(Vakantiekenmerken, Perioden, GemiddeldPerDeelnemer_6)



### filter observations ###
dataset_adjusted <- dataset_adjusted %>% 
  filter(Perioden == "2022JJ00")
